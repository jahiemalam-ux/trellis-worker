...
def run_job(jid, inp):
    ...
    try:
        ...
        if not b64s:
            ...
            import txt2img
            # Multi-view: generate several angles so TRELLIS fuses them instead of
            # inventing the far side from one view (the audit's top-payoff fix).
            views = inp.get("t2i_views")
            if views:
                if views is True or views == "auto":
                    views = None  # txt2img picks a sensible default set
                step("txt2img: generating multiple views")
                b64s = txt2img.generate_views_b64(
                    prompt,
                    views=views,
                    seed=int(inp.get("seed", 42)),
                    steps=int(inp.get("t2i_steps", 4)),
                    size=int(inp.get("t2i_size", 512)),
                )
                multiview_t2i = True
                prompt_image_b64 = b64s[0]  # keep the first for preview
                step(f"txt2img: {len(b64s)} views generated")
            else:
        ...
        if inp.get("subject_check", True):
            try:
                ...
                if len(images) > 1:
                    # Multi-view: can't reroll a single image; just log each view's
                    # verdict so a bad view is visible in the job log.
                    for i, im in enumerate(images):
                        single, info = subject_check.analyze(im)
                        step(f"subject check view {i}: single={single} {info}")
                else:
