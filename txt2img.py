...
# View directions for pseudo-multi-view generation. TRELLIS fuses DINOv3 features
# across these, filling in the far side it would otherwise have to hallucinate
# from a single view. IMPORTANT: SDXL generates each view independently, so they
# are NOT geometrically consistent the way true multi-view diffusion (e.g. MVDream,
# Zero123) would be. Same seed + directional phrasing minimises drift, but this is
# a pragmatic approximation, not view-consistent generation.
VIEW_DIRECTIONS = {
    "front": "front three-quarter view, front of the object facing camera",
    "back": "rear three-quarter view, back of the object facing camera",
    "side": "direct side profile view, side of the object facing camera",
    "front34": "front 3/4 view from slightly above",
    "back34": "rear 3/4 view from slightly above",
}
DEFAULT_VIEWS = ["front34", "back34", "side"]


def generate_views(prompt, views=None, seed=42, steps=4, size=512,
                   style_wrap=True):
    """
    Generate several angles of the same subject for TRELLIS multi-view fusion.

    Uses a FIXED seed across views on purpose: it keeps colour, proportion and
    styling as stable as SDXL allows, so the views at least look like the same
    object. Returns a list of PIL images.
    """
    views = views or DEFAULT_VIEWS
    imgs = []
    base = prompt.strip()
    for v in views:
        direction = VIEW_DIRECTIONS.get(v, v)
        # Swap the generic "3/4 view" suffix wording for this view's direction.
        vp = f"{base}, {direction}"
        if style_wrap:
            suffix = POSITIVE_SUFFIX.replace("3/4 view, ", "")
            vp = f"{vp}, {suffix}"
        log(f"view '{v}': generating")
        img = generate(vp, seed=seed, steps=steps, size=size, style_wrap=False)
        imgs.append(img)
    return imgs


def generate_views_b64(prompt, **kw):
    out = []
    for img in generate_views(prompt, **kw):
        buf = io.BytesIO()
