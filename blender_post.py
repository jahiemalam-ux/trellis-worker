...
def create_glass_material():
    """Create a simple but effective transparent glass material."""
    ...
    nt.links.new(bsdf.outputs["BSDF"], out.inputs["Surface"])
    mat.blend_method = "HASHED" if hasattr(mat, "blend_method") else "BLEND"
    if hasattr(mat, "shadow_method"):
        mat.shadow_method = "HASHED"
    log("created WindowGlass material (transmission=1.0, IOR=1.45)")
    return mat


def detect_and_assign_glass(obj, basecolor_img=None):
    """
    Heuristic window detection for cars:
    - Upper part of the bounding box
    - Mostly vertical faces (not roof, not underbody)
    - Dark / low-saturation in the baked texture
    Assigns a transparent glass material to those faces.
    """
    ...
    if not obj.data.materials:
        obj.data.materials.append(bpy.data.materials.new("Fallback"))
    ...
    upper_threshold = z_min + 0.42 * z_range

    use_color = basecolor_img is not None and basecolor_img.size[0] > 0
    pixels = None
    w = h = 0
    if use_color:
        try:
    ...
    for poly in mesh.polygons:
        ...
        if use_color and uv_layer is not None:
            ...
            if uvs:
                ...
                is_dark = (luminance < 0.28) and (saturation < 0.22)

        if is_dark:
            poly.material_index = glass_idx
            assigned += 1

    log(f"glass assignment: {assigned} faces marked as windows")
    return assigned
...
def main():
    ...
    try:
        basecolor_img = None
        for img in bpy.data.images:
            if img.name.startswith("BakedBaseColor"):
                basecolor_img = img
                break
        n_glass = detect_and_assign_glass(tgt, basecolor_img)
        log(f"window glass faces assigned: {n_glass}")
    except Exception as e:
        log(f"glass pass failed (continuing): {e}")
